...
calibration_dir = Path("/home/user/Projects/yarbiv/rmp_lab/uris_gui")
...
def transform_point_uri_to_ayal(P_in_uri_frame):
    ...
    """
    T_uri_to_ayal = gemini.pose_to_T(P_URI_TO_AYAL)
    T_in_uri_frame = gemini.pose_to_T(P_in_uri_frame)
    T_in_ayal_frame = np.linalg.inv(T_uri_to_ayal) @ T_in_uri_frame
    P_in_ayal_frame = gemini.T_to_pose(T_in_ayal_frame)
...
def run_dual_robot_scenario(uri_robot, ayal_robot):
    """
...
if __name__ == "__main__":
    ...
    # RUN SCENARIO
    run_dual_robot_scenario(uri, ayal)
