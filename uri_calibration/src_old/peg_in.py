...
Slow-seek phase (no LQR):
    ...
    Finish recomputes base_to_base via gemini.calculate_ayal_in_uri and
    APPENDS a new sample to uris_gui/calibration_samples.json (the same file
    panel_calibrate writes to, consumed by gemini.calculate_optimal_calibration).
...
CALIBRATION_FILE          = os.path.join(_RMP_LAB_ROOT, "uris_gui", "calibration.json")
CALIBRATION_SAMPLES_FILE  = os.path.join(_RMP_LAB_ROOT, "uris_gui", "calibration_samples.json")
...
def wrench_to_tcp(wrench_base: list[float], pose: list[float]) -> list[float]:
    return list(gemini.wrench_trans(wrench_base, pose, base_to_tcp=True, include_translation=False))
...
def ayal_pose_in_uri_base(ayal_pose: list[float], base_to_base: list[float]) -> list[float]:
    ...
    return list(gemini.T_to_pose(T_uri_ayalbase @ T_ayalbase_ayaltcp))
...
def load_calibration() -> list[float]:
    try:
        with open(CALIBRATION_FILE) as f:
            cal = json.load(f)
        if isinstance(cal, list) and len(cal) == 6:
            print(f"loaded calibration from {CALIBRATION_FILE}: {cal}")
            return list(cal)
        print(f"calibration.json has unexpected shape {cal!r}; using default")
    except FileNotFoundError:
        print(f"calibration.json not found at {CALIBRATION_FILE}; using default")
    return list(DEFAULT_BASE_TO_BASE)
...
def append_calibration_sample(uri_pose: list[float], ayal_pose: list[float], derived_b2b: list[float]) -> int:
    ...
    Schema matches what panel_calibrate writes / gemini.calculate_optimal_calibration
...
def approach_to_near_mate(
    ...
    try:
        for step in range(args.steps):
            ...
            mate_pose = list(gemini.calculate_mirror_position(uri_pose, base_to_base, flip_trans=True))
...
def spiral_orientation_search(
    ...
    R_base = gemini.rotvec_to_R(base_pose[3:6])
    ...
    for direction in (+1, -1):
        ...
        for k in range(1, max_steps + 1):
            ...
            R_target = R_base @ gemini.rotvec_to_R(pert_rotvec)
            target = list(base_pose[:3]) + list(gemini.R_to_rotvec(R_target))
        ...
        while True:
            ...
            while True:
                ...
                if choice in ("f", "finish"):
                    uri_pose = list(uri.recieve.getActualTCPPose())
                    ayal_pose = list(ayal.recieve.getActualTCPPose())
                    new_b2b = list(gemini.calculate_ayal_in_uri(uri_pose, ayal_pose))
